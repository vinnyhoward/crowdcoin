...
export default class RequestNew extends Component {
  state = {
    ...
    onSubmit = async event => {
      ...
      const { description, value, receipt } = this.state;
      ...
      try {
        ...
        await campaign.methods.createRequest( description, web3.utils.toWei( value, 'ether' ), receipt )
...
<Layout>
  ...
  <Form onSubmit={ this.onSubmit } error={ !!this.state.errorMessage }>
    ...
    <Form.Field>
      ...
      <Input type='text' value={ this.state.receipt } onChange={ event=> this.setState({ receipt: event.target.value })} />
    </Form.Field>
